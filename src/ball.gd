...
func deflect_from_paddle(collision: KinematicCollision2D) -> Vector2:
	...
	# adjust paddle from origin in center to origin on left
	var paddle_x = collision.get_collider().global_position.x - (paddle_width / 2)
	var hit_x = collision.get_position().x
	# distance from center of paddle as range between 0 and 1
	var distance_from_center = (hit_x - paddle_x) / paddle_width
